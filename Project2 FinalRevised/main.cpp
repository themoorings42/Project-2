...
//just shows the options a user can choose
void Menu();

//gets the uders menu option choice
int getN();

//shows the number the user entered to exit the game
void def(int);

//this is the game on beginner mode, you get 10 tries to solve the game and have to guess 3 numbers
void beginner(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[10], int& num_games);

//this is the game on intermediate mode, you get 5 tries to solve the game and have to guess 4 numbers
void intermediate(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[10], int& num_games);

//this is the game on advanced mode, you get 5 tries to solve the game and have to guess 3 numbers
void advanced(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[10], int& num_games);

//this shows the high scores of the current game
void highscores(vector<HighScores> &player_list, int num_games);

//this fills the high score list with the default values for the users data
void fillscores(vector<HighScores> &player_list, int num_games, fstream& to_file);
...
void save_scores(vector<HighScores> &player_list, fstream& to_file, int users_count);

//displays the scores saved to a file from previous games
void view_saved_scores(fstream& to_file);
...
int main(int argv,char *argc[])
{
	...
	vector<HighScores> player_list;
	fstream to_file;
	
	//opens up notepad to display the instructions
	//only works on windows systems as it uses the system cmd
	//Dont know how to get it to work on the computer systems
	...
	do{
		...
		switch(inN)
		{
			...
			case 4:
				{
					highscores(player_list, users_count);
					break;
				}
						
			case 5:
				{
					view_saved_scores(to_file);
					break;
				}
    ...
    save_scores(player_list, to_file, users_count);
...
void fillscores(vector<HighScores> &player_list, int num_games, fstream& to_file)
{
    ...
    {
        player_list[i].set_level("000");
        player_list[i].set_attempts(0);
        
        //for(int j = 0; j < 3; j++)
        //{
        player_list[i].set_initials("000");   
        //}
	}
}
...
void highscores(vector<HighScores> &player_list, int num_games)
{
    cout << setw(25) << "TOP " << num_games << " SCORES\n";
    ...
    for(int i = 0; i < num_games; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            cout << player_list[i].get_initials(j);
        }
        
        cout << setw(11) << player_list[i].get_level() << setw(8) << player_list[i].get_attempts() << endl;   
    }    
}   
    
void beginner(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[], int& num_games)
{ 
    ...
    {        
        ...
        {//for loop starting bracket
			...
			{     			
                ...
                for(int i = 0; i < 3; i++)
                {                	
                	cin >> temporary_character[i];
                }
				
				if(player_list.capacity() <= num_games)
				{
					player_list.resize(player_list.size() + 1);
     			    
					 ++num_games;
				}

                player_list[users_count].set_initials(temporary_character); 
                player_list[users_count].set_attempts(user_tries + 1);
                player_list[users_count].set_level(set_level);
...
void intermediate(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[], int& num_games)
{    	
...
void advanced(int& users_count, vector<HighScores> &player_list, string set_level, bool matched_choice[], int& num_games)
{
    char init1,init2,init3,num1,num2,num3,ans1=32,ans2=32,ans3=32,replay;
    int ran1,ran2,ran3,num1_value,num2_value,num3_value;
...
void save_scores(vector<HighScores> &player_list, fstream &to_file, int users_count)
{
	...
	{
		to_file << player_list[i].get_attempts() << " ";
		
		for(int j = 0; j < 3; j++)
			to_file << player_list[j].get_initials(i) << " ";
			
		to_file << player_list[i].get_level() << endl;
	}
